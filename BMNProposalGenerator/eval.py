...
def run_evaluation(opt, proposal_filename,
                   max_avg_nr_proposals=100, 
                   tiou_thresholds=np.linspace(0.5, 0.95, 10),
                   subset='validation'):

    anet_proposal = ANETproposal(opt, proposal_filename,
...
def plot_metric(opt,average_nr_proposals, average_recall, recall, tiou_thresholds=np.linspace(0.5, 0.95, 10)):
    ...
    plt.savefig(opt['output_path']+'/'+opt["save_fig_path"])

def evaluation_proposal(opt):
    if opt['inference_set'] == 'validation':
        outfile = opt['output_path']+'/'+opt["result_file"].replace('.json', '-validation.json')
    else:
        outfile = opt['output_path']+'/'+opt["result_file"].replace('.json', '-test.json')
    uniform_average_nr_proposals_valid, uniform_average_recall_valid, uniform_recall_valid = run_evaluation(
        opt,
        outfile,
        max_avg_nr_proposals=opt['max_proposals'],
        tiou_thresholds=np.linspace(0.5, 0.95, 10),
        subset=opt['inference_set'])
    
    plot_metric(opt,uniform_average_nr_proposals_valid, uniform_average_recall_valid, uniform_recall_valid)
    print( "AR@1 is {:0.2f}%\t".format(np.mean(uniform_recall_valid[:,0])*100))
    print( "AR@5 is {:0.2f}%\t".format(np.mean(uniform_recall_valid[:,4])*100))
    print( "AR@10 is {:0.2f}%\t".format(np.mean(uniform_recall_valid[:,9])*100))
    print( "AR@-1 is {:0.2f}%\t".format(np.mean(uniform_recall_valid[:,-1])*100))