...
def train_BMN(data_loader, model, optimizer, epoch, bm_mask):
    ...
    for n_iter, (input_data, label_confidence, label_start, label_end) in enumerate(data_loader):
        ...
        print(
            "BMN training loss(epoch %d - iteration %d/%d): tem_loss: %.03f, pem class_loss: %.03f, pem reg_loss: %.03f, total_loss: %.03f" % (
                epoch, n_iter, len(data_loader), epoch_tem_loss / (n_iter + 1),
...
def BMN_Train(opt):
    ...
    bm_mask = get_mask(opt["observation_window"], opt["max_duration"])
...
def BMN_inference(opt):
    ...
    set_ = VideoDataSet(opt, subset= opt['inference_set'])

    test_loader = torch.utils.data.DataLoader(set_,
                                              batch_size=1, shuffle=False,
                                              num_workers=8, pin_memory=False, drop_last=False)

    observation_window = opt["observation_window"]
    max_duration = opt["max_duration"]
    with torch.no_grad():
        for idx, input_data in test_loader:
            print(f"Processing {idx.item()}/{len(test_loader)}")
            ...
            # 遍历起始分界点与结束分界点的组合
            new_props = []
            for idx in range(max_duration):
                for jdx in range(observation_window):
                    start_index = jdx
                    end_index = start_index + idx+1
                    if end_index < observation_window and start_bins[start_index] == 1 and end_bins[end_index] == 1:
                        xmin = start_index/observation_window
                        xmax = end_index/observation_window
            ...
            new_df.to_csv(opt['output_path']+"/BMN_results/" + video_name + ".csv", index=False)
...
def main(opt):
    if opt["mode"] == "train":
        ...
        if not os.path.exists(opt['output_path']+"/BMN_results"):
            os.makedirs(opt['output_path']+"/BMN_results")
        print("Inference start")
        BMN_inference(opt)
    elif opt["mode"] == 'postprocessing':
