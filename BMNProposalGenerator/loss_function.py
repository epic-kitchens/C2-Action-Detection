...
def get_mask(observation_window, max_duration):
    bm_mask = []
    for idx in range(max_duration):
        mask_vector = [1 for i in range(observation_window - idx)
...
def bmn_loss_func(pred_bm, pred_start, pred_end, gt_iou_map, gt_start, gt_end, bm_mask):
    ...
    pem_reg_loss = pem_reg_loss_func(pred_bm_reg, gt_iou_map, bm_mask) #nan
    pem_cls_loss = pem_cls_loss_func(pred_bm_cls, gt_iou_map, bm_mask) #nan
