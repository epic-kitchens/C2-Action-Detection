...
class ANETproposal(object):
    ...
    def __init__(self, opt, proposal_filename=None,
        ...
        self.ground_truth, self.activity_index = self._import_ground_truth(opt, subset)
    ...
    def _import_ground_truth(self, opt, subset):
        annotations = pd.read_csv(join(opt["path_to_dataset"], subset + '.csv'),
                                  names=['id', 'video', 'start', 'stop', 'verb', 'noun', 'action'], index_col='id')

        gt = pd.DataFrame()
        gt['video-id']=annotations['video']
        gt['t-start']=annotations['start']/opt['fps']
        gt['t-end']=annotations['stop']/opt['fps']
        gt['label']=annotations['action']

        return gt, gt['label'].values

    def _import_ground_truth_old(self, ground_truth_filename):
    ...
    def evaluate(self):
        ...
        recall, avg_recall, proposals_per_video = average_recall_vs_avg_nr_proposals(
                ...
                tiou_thresholds=self.tiou_thresholds, opt=self.opt)

        area_under_curve = np.trapz(avg_recall, proposals_per_video)

        if self.verbose:
            print('[RESULTS] Performance on ActivityNet proposal task.')
            print('\tArea Under the AR vs AN curve: {:0.2f}%'.format(100.*float(area_under_curve)/proposals_per_video[-1]))
...
def average_recall_vs_avg_nr_proposals(ground_truth, proposals,
                                       max_avg_nr_proposals=None,
                                       tiou_thresholds=np.linspace(0.5, 0.95, 10), opt=None):
    ...
    for videoid in video_lst:
        ...
        if opt['noperminute']:
            ...
            nr_proposals = int(np.minimum(max_avg_nr_proposals*length_dict[videoid]/opt['fps']/60, this_video_proposals.shape[0]))
            all_proposals_queried += this_video_proposals.shape[0]
    ...
    if opt['noperminute']:
        ...
        pcn_lst = np.arange(1, 101) / 100.0 * all_proposals_queried / total_nr_proposals
...
def average_recall_vs_avg_nr_proposals_bak(ground_truth, proposals,
                                       max_avg_nr_proposals=None,
                                       tiou_thresholds=np.linspace(0.5, 0.95, 10)):
    """ Computes the average recall given an average number 
        of proposals per video.
