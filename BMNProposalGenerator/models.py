...
class BMN(nn.Module):
    def __init__(self, opt):
        ...
        self.observation_window = opt["observation_window"]
    ...
    def _boundary_matching_layer(self, x):
        input_size = x.size()
        # BS x C x N x D x T
        out = torch.matmul(x, self.sample_mask).reshape(input_size[0],input_size[1],self.num_sample,self.max_duration,self.observation_window)
        return out
